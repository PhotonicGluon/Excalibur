...
- `hmac_validation`: The proof-of-possession (PoP) token computed according to the [WebSocket PoP token generation process](../server-api/authentication.md#proof-of-possession-pop).
...
If `encrypted` is `true`, messages will be sent in the Excalibur Encryption Format (ExEF). Do remember to decrypt the messages using the [SRP master key](../server-api/authentication.md).
