...
- `hmac_validation`: The proof-of-possession (PoP) token computed according to the [WebSocket PoP token generation process](./authentication/subsequent).
...
:::warning[Duplicate Connections]

If the same `auth_token` is used to open another listener connection while the existing one has not yet disconnected, the second connection will close with code `4000`.
