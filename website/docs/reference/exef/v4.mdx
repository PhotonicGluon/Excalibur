...
| `6`     | **Chunk Size Exponent**, $E$ | 1            | A 1-byte unsigned integer representing the base-2 exponent of the chunk size. Note that the plaintext chunk size is $C = 2^E$ bytes. **Must** satisfy $4 \leq E \leq 30$ (16 B to 1 GiB).                        |
...
2. Choose the chunk size exponent $E$ (with $4 \leq E \leq 30$), let $C = 2^E$, and compute $N = \lceil \frac{L_p}{C} \rceil$. Reject if $N \geq 2^{32}$.
...
2. From the header, obtain the chunk size exponent $E$ and chunk count $N$. Assert $4 \leq E \leq 30$, $8 \leq L_p \leq 2^{64}-1$, $\mathrm{PADME}(L_p - 8) = L_p - 8$, $N \geq 1$, $N = \lceil \frac{L_p}{C} \rceil$, _in that order_. Reject on any mismatch.
