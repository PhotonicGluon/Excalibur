...
description: Specification for ExEF version 3.
---
...
We do not directly use the vault key ($K$) for encrypting data. Instead, two keys are derived from the vault key using HKDF.
...
The header MAC is ***not*** used for authentication. It is only used as a quick verification on the provided vault key $K$. **It must _not_ be used for authentication**.
...
After verifying that the vault key $K$ is (likely) correct using the header MAC and the MAC key $k_m$, the crypto key $k_c$ is used to decrypt the ciphertext. Verification of the decryption is done using the footer tag (which is the AES-256-GCM authentication tag).
