...
- [**API Only**](http://localhost:3000/download?type=server): This is the main Excalibur API server.
    - It does not have a user interface; it just provides the API for clients to connect to.
- [**With PWA**](http://localhost:3000/download?type=server-pwa): This is the Excalibur API server with a Progressive Web Application (PWA).
...
This will create a new folder `excalibur-files` in the current working directory.
