...
- A new table `security.opaque` was added, containing fields relating to the [OPAQUE-3DH](/docs/server-api/authentication/index.mdx) protocol, which are:
...
Along with the addition of the [OPAQUE-3DH](/docs/server-api/authentication/index.mdx) protocol for authentication, a new option `--auth-protocol` has been added to the `excalibur user add` command. It defaults to the new OPAQUE authentication mechanism; for SRP compatibility, specify `--auth-protocol=SRP`.
