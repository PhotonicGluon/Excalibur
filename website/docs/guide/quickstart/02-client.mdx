...
<Tabs groupId="platform">
    ...
    The Ubuntu client can be obtained as either a `.deb` file, a `.snap` file, or an `.AppImage` file.

    <Tabs groupId="ubuntu-file">
        ...
        <TabItem value="snap" label=".snap">
            We assume that [`snap`](https://snapcraft.io/docs/installing-snap-on-ubuntu) is installed on your machine.
            1. Download the `.snap` package from <a href="/download?id=app-linux-snap" target="_blank">here</a>.
            2. In the same directory as the downloaded file, run
               ```bash
               ...
               :::note What is `--dangerous`?
               
               The `--dangerous` flags installs the given snap file even if there are no pre-acknowledged signatures for it, which is the case for Excalibur.
               :::
