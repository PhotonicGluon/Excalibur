...
- <a href="/download?type=server" target="_blank"><b>API Only</b></a>: This is the main Excalibur API server.
    - It does not have a user interface; it just provides the API for clients to connect to.
- <a href="/download?type=server-pwa" target="_blank"><b>With PWA</b></a>: This is the Excalibur API server with a Progressive Web Application (PWA).
...
Click on any the above links to download the server distributable, which is a `.whl` file. Once done, we can install the server.
