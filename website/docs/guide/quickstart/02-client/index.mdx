...
Setting up the user on the server is vulnerable to leaking of the vault key. This is because, if you are strictly following the [security model](/docs/reference/02-security-model.md) of Excalibur, this method would involve entering the vault key ***in the clear*** on the server, which is assumed to read and modify anything on it.
