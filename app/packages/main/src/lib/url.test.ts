...
describe("getURLEncodedPath", () => {
    ...
    describe("with query", () => {
        it("query excluded by default", () => {
            expect(getURLEncodedPath("http://example.com/upload?force=true")).toBe("/upload");
        });

        it("no query string", () => {
            expect(getURLEncodedPath("http://example.com/test", true)).toBe("/test");
        });

        it("with query string", () => {
            expect(getURLEncodedPath("http://example.com/upload?force=true", true)).toBe("/upload?force=true");
        });

        it("multiple query params", () => {
            expect(getURLEncodedPath("http://example.com/upload?force=true&x=1", true)).toBe("/upload?force=true&x=1");
        });

        it("empty query string", () => {
            expect(getURLEncodedPath("http://example.com/upload?", true)).toBe("/upload");
        });
    });
});
