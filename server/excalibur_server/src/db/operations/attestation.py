...
def get_attestations(root_id: UUID, *, from_gen: int | None = None, to_gen: int | None = None) -> list[Attestation]:
    """
    ...
    statement = select(Attestation).where(Attestation.root_id == root_id)
    if from_gen is not None:
        statement = statement.where(Attestation.generation >= from_gen)
    if to_gen is not None:
        statement = statement.where(Attestation.generation <= to_gen)

    statement = statement.order_by(Attestation.generation.desc())
...
def get_latest_attestation(root_id: UUID) -> Attestation | None:
    """
