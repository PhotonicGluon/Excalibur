...
from excalibur_server.src.auth.opaque.oprf import OPRF
...
def test_oprf(test_vector):
    ...
    # Test `blind()`
    out_blind, out_blinded_element = OPRF.blind(input, blind=blind)
    assert out_blind == blind
    assert out_blinded_element.to_bytes() == blinded_element_hex

    # Test `blind_evaluate()`
    out_evaluated_element = OPRF.blind_evaluate(SK_SCALAR, out_blinded_element)
    assert out_evaluated_element.to_bytes() == evaluated_element_hex

    # Test `finalize()`
    our_output = OPRF.finalize(input, blind, out_evaluated_element)
