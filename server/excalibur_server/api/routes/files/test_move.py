...
class TestMoveOld:
    @pytest.fixture()
    def move_folder(self, test_user_vault_folder: Path):
        # Create a folder that contains the test items
    ...
    def test_move(self, auth_client: TestClient, test_user_vault_folder: Path, move_folder: Path):
    ...
    def test_move_with_encrypted_path(self, auth_client: TestClient, move_folder: Path):
        from base64 import b64encode
    ...
    def test_path_traversal(self, auth_client: TestClient, move_folder: Path):
    ...
    def test_already_exists(self, auth_client: TestClient, move_folder: Path):
        (move_folder / "m-file").touch()
        response = auth_client.post("/api/files/move/move-folder/m-file", json="move-folder")
        assert response.status_code == 409  # Item already exists

    def test_destination_nonexistent(self, auth_client: TestClient, move_folder: Path):
        (move_folder / "m-file").touch()
