...
def example_file(tmp_path_factory: pytest.TempPathFactory) -> Path:
...
def test_no_auth(example_file: Path):
...
def test_file_already_exists(auth_client: TestClient, test_user_vault_folder: Path):
    uuid = uuid4().hex
    uploaded_file = test_user_vault_folder / f"test-{uuid}.txt.exef"
